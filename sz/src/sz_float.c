...
size_t dataLength, double realPrecision, float valueRangeSize, float medianValue_f)
{
    ...
    double* precisionTable2 = (double*)malloc(sizeof(double) * quantization_intervals);
    struct ABSTopLevelTableWideInterval levelTable;

	printf("errorBound = %f\n",realPrecision);
    struct timeval start,end;

    gettimeofday(&start,NULL);
    float intersect_params=0.5;
    //printf("No problems!\n");
    for(int i=0; i<exe_params->intvCapacity; i++){
        double test = (2*intersect_params*i+1)*realPrecision;
        precisionTable[i] = test;
    }
    float adjustNum = (precisionTable[0]+precisionTable[exe_params->intvCapacity-1])/2;
    for(int i=0; i<exe_params->intvCapacity; i++){

        precisionTable2[i] =precisionTable[i]-adjustNum;
    }

    //double smallest_precision = precisionTable[0], largest_precision = precisionTable[quantization_intervals-1];

    ABSMultiLevelCacheTableWideIntervalBuild(&levelTable, precisionTable, exe_params->intvCapacity, realPrecision, intersect_params);
    gettimeofday(&end,NULL);

    printf("the time of making table：%lf\n",end.tv_sec-start.tv_sec+((double)(end.tv_usec-start.tv_usec))/1000000);
     ...
     for(i=2;i<dataLength;i++)
    {
        ...
        state = ABSMultiLevelCacheTableWideIntervalGetIndex(predAbsErr, top, base, tables, bitArray);
        //printf("%f\n",predAbsErr);
...
//        }
        ...
        if(state){
        	type[i]=state;
        	//pred = pred+precisionTable[state]-adjustNum;
            pred = pred+precisionTable2[state];
    ...
    gettimeofday(&end,NULL);
    printf("the time of handling：%lf\n",end.tv_sec-start.tv_sec+((double)(end.tv_usec-start.tv_usec))/1000000);
	//printf("amendNum = %ld\n",amendNum);
    printf("hit = %f\n", (float)hitNum/dataLength);
...
unsigned char * SZ_compress_float_2D_MDQ_nonblocked_with_blocked_regression(float *oriData, size_t r1, size_t r2, double realPrecision, size_t * comp_size){
	...
	if(use_mean){
		...
		type = result_type;
		int intvCapacity_sz = intvCapacity - 1;
		for(size_t i=0; i<num_x; i++){
			current_blockcount_x = (i < split_index_x) ? early_blockcount_x : late_blockcount_x;
			offset_x = (i < split_index_x) ? i * early_blockcount_x : i * late_blockcount_x + split_index_x;
				...
				if(use_reg)
				{
					...
					for(size_t ii=0; ii<current_blockcount_x - 1; ii++){
						for(size_t jj=0; jj<current_blockcount_y - 1; jj++){
							...
							diff = curData - pred + adjustNum;
							state = ABSMultiLevelCacheTableWideIntervalGetIndex(diff, top, base, tables, bitArray);
							if (state){
								hitNum++;
								type[index] = state;
								pred = pred + precisionTable2[state];
					...
					for(size_t ii=0; ii<current_blockcount_x - 1; ii++){
						for(size_t jj=0; jj<current_blockcount_y; jj++){
							...
							else
							{
								...
								diff = curData - pred2D + adjustNum;
								state = ABSMultiLevelCacheTableWideIntervalGetIndex(diff, top, base, tables, bitArray);
								if (state!=0 && state != intvCapacity_sz){
									hitNum++;
									type[index] = state;
									*cur_pb_pos = pred2D + precisionTable2[state];
									//ganrantee comporession error against the case of machine-epsilon
...
unsigned char * SZ_compress_float_3D_MDQ_nonblocked_with_blocked_regression(float *oriData, size_t r1, size_t r2, size_t r3, double realPrecision, size_t * comp_size){
							...
							for(size_t jj=0; jj<current_blockcount_y; jj++){
								for(size_t kk=0; kk<current_blockcount_z; kk++){
									...
									else
									{
										...
										diff = curData - pred3D + adjustNum;
										state = ABSMultiLevelCacheTableWideIntervalGetIndex(diff, top, base, tables, bitArray);
										if (state!=0 && state != intvCapacity_sz){
											hitNum++;
											type[index] = state;
											*cur_pb_pos = pred3D + precisionTable2[state];
											//ganrantee comporession error against the case of machine-epsilon
											if(fabs(curData - *cur_pb_pos)>tmp_realPrecision){
												hitNum--;
												type[index] = 0;
	...
	//printf("mean = %f\n",(float)meanNum/(r1*r2*r3));

	printf("hit = %f\n",(float)hitNum/(r1*r2*r3));
	free(prediction_buffer_1);
	free(prediction_buffer_2);
