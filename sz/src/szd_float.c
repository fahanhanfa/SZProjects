...
void decompressDataSeries_float_1D(float** data, size_t dataSeriesLength, TightDataPointStorageF* tdps) 
{
	...
	float intersect_params=0.9;
	double* precisionTable = (double*)malloc(sizeof(double) * tdps->intervals);
	double* precisionTable2 = (double*)malloc(sizeof(double) * tdps->intervals);

	struct timeval start,end;

	gettimeofday(&start,NULL);

	for(int i=0; i<tdps->intervals; i++){
		double test = (2*intersect_params*i+1)*tdps->realPrecision;
		precisionTable[i] = test;
	}
	float adjustNum = (precisionTable[0]+precisionTable[tdps->intervals-1])/2;
	for(int i=0; i<tdps->intervals; i++){
	...
	*data = (float*)malloc(sizeof(float)*dataSeriesLength);
	...
	for (i = 0; i < dataSeriesLength; i++) {	
		...
		default:
			...
			predValue = (*data)[i-1];
			(*data)[i] = predValue + precisionTable2[type_];
			break;
		}
		//printf("%.30G\n",(*data)[i]);
	}
